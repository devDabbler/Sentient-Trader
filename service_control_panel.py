...
SERVICES = {
    ...
    "Pump.fun Gambler": {
        "name": "sentient-pumpfun-gambler",
        "description": "Real-time pump.fun bonding curve monitoring (new tokens, graduations)",
        "emoji": "🎲",
        "category": "crypto",
        "interval_key": "scan_interval_seconds",
        "interval_default": 5,
        "interval_min": 2,
        "interval_max": 60  # Max 1 minute between scans
    },
...
def clear_analysis_requests() -> bool:
    ...
    try:
        ...
        # Set session state flag to indicate data changed
        st.session_state['_analysis_requests_cleared'] = time.time()
...
def clear_analysis_results() -> bool:
    ...
    try:
        ...
        # Invalidate Streamlit cache - use try/except for robustness
        try:
            get_analysis_results.clear()
        except Exception:
            pass  # Cache may not exist yet
        
        # Set session state flag to indicate data changed (forces UI to re-read)
        st.session_state['_analysis_results_cleared'] = time.time()
...
def get_service_status(service_name: str) -> dict:
    ...
    if platform.system().lower().startswith('win'):
        windows_name_map = {
            ...
            'sentient-pumpfun-gambler': 'SentientPumpfunGambler',
...
def get_service_logs(service_name: str, lines: int = 100) -> str:
    ...
    log_map = {
        ...
        'sentient-pumpfun-gambler': [
            'logs/pumpfun_gambler.log'
        ],
...
def main():
    ...
    with tab_workflow:
        ...
        with st.expander("🎛️ Preset Modes", expanded=False):
            presets = [
                ("🪙 Crypto", ["DEX Launch Monitor", "DEX Fast Position Monitor", "Pump.fun Gambler", "Crypto Breakout Monitor", "AI Crypto Trader", "Discord Approval Bot", "Analysis Queue Processor"]),
                ("📈 Stocks", ["Stock Monitor", "AI Stock Trader", "ORB FVG Scanner", "Discord Approval Bot", "Analysis Queue Processor"]),
                ("👀 Monitors", ["DEX Launch Monitor", "DEX Fast Position Monitor", "Pump.fun Gambler", "Crypto Breakout Monitor", "Stock Monitor", "ORB FVG Scanner", "Analysis Queue Processor"]),
        ...
        for svc_label, svc_info in SERVICES.items():
            ...
            with st.container():
                ...
                if svc_info.get("interval_key"):
                    with st.expander(f"⚙️ Settings - {display_name}"):
                        ...
                        # ============================================================
                        if service_name == "sentient-stock-monitor":
                            st.markdown("---")
                            st.markdown("**🔍 Stock Discovery Universe**")
                            st.caption("Discover trading opportunities outside your watchlist")
                        ...
                        # ============================================================
                        # DISCOVERY INFO (Crypto Breakout Monitor)
                        # ============================================================
                        if service_name == "sentient-crypto-breakout":
                            st.markdown("---")
                            st.markdown("**🔍 Discovery Settings**")
                            st.info("""
                            **Crypto Breakout Monitor** automatically discovers new opportunities:
                            
                            ✅ **Pre-listing Scanner** - Finds new coins before they list on Kraken
                            ✅ **Auto-add to Watchlist** - New discoveries automatically added
                            ✅ **Volume Breakouts** - Detects unusual volume spikes
                            ✅ **Momentum Plays** - Catches trending coins early
                            
                            Discovery is **always enabled** for crypto. The monitor scans both your watchlist and discovers new opportunities automatically.
                            """)
                            
                            # Show current watchlist size
                            try:
                                ...
                                st.metric("Current Watchlist Size", len(crypto_watchlist))
                            except:
...
""", unsafe_allow_html=True)
    ...
    if analysis_results:
        ...
        with button_col1:
            if st.button("🔄 Refresh Now", key="force_refresh_results"):
                get_analysis_results.clear()
                st.toast("🔄 Refreshing...")
                st.rerun()
        ...
        with button_col2:
            if st.button("🗑️ Clear All Results", key="clear_all_results"):
                if clear_analysis_results():
                    ...
                    # Force cache clear and update display immediately
                    try:
                        get_analysis_results.clear()
                    except:
                        pass
